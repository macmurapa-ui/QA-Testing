/**
 * HTM Clone - Test Run 011
 * Existing User Check + Invite Creation on Branch 2481 (Mac 1300326)
 * — invite to be used as the subject of Test Run 012 (Invite Revoke)
 *
 * Steps:
 *   [1/3] Verify session via auth.json
 *   [2/3] Existing User Check — determine next available mac.murapa[N][DDMMYY] email
...
(async () => {
  console.log('═══════════════════════════════════════════════════════');
  console.log(' HTM Clone - Test Run 011: Existing User Check + Invite');
  ...
  while (n <= 20) {
    ...
    await page.screenshot({ path: `${SS_DIR}/011_user_check_n${n}.png`, fullPage: false });

    const found = await page.evaluate(email => document.body.innerText.includes(email), candidate);
    if (found) {
      console.log(`      FOUND — already exists. Incrementing N.`);
      n++;
    } else {
  ...
  await page.screenshot({ path: `${SS_DIR}/011_invites_page.png`, fullPage: false });

  await page.click('text=New invite');
  await page.waitForLoadState('domcontentloaded', { timeout: 15000 });
  await page.waitForTimeout(800);
  await page.screenshot({ path: `${SS_DIR}/011_new_invite_form.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SS_DIR}/011_invite_form_filled.png`, fullPage: false });

  // Scroll to and JS-click Send Invite button
  await page.evaluate(() => {
  ...
  await page.screenshot({ path: `${SS_DIR}/011_invite_result.png`, fullPage: false });
  ...
  if (isSuccess) {
    ...
    console.log(' Branch : ' + BRANCH_NAME + ' (ID: ' + BRANCH_ID + ')');
    console.log(' Note   : Use this email in Test Run 012 (Invite Revoke)');
  } else {
    console.log(' FAILED: Invite not confirmed — check 011_invite_result.png');
    process.exitCode = 1;
  }
  console.log('═══════════════════════════════════════════════════════');
