/**
 * HTM Clone - Test Run 020
 * Property Creation in Dashboard — VACANT
 *
 * Steps:
 ...
 *   [5/6] Fill form: postcode M1 1AE → lookup → select Apartment 1 → status: Vacant
...
const TEST_ADDRESS  = 'Apartment 1, 113 Newton Street, Manchester, M1 1AE';
const STATUS        = 'vacant';
...
(async () => {
  console.log('═══════════════════════════════════════════════════════');
  console.log(' HTM Clone - Test Run 020: Property Creation (Vacant)');
  ...
  await page.screenshot({ path: `${SS_DIR}/020_dashboard.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SS_DIR}/020_landlords.png`, fullPage: false });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: `${SS_DIR}/020_landlord_page.png`, fullPage: false });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: `${SS_DIR}/020_property_form.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SS_DIR}/020_postcode_results.png`, fullPage: false });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: `${SS_DIR}/020_address_selected.png`, fullPage: false });
  ...
  // Status: Vacant — radio is visually hidden by fancy-radio CSS; use JS click
  await page.evaluate(() => {
    const el = document.querySelector('#property_create_status_vacant');
    if (el) el.click();
  });
  console.log('      Status: Vacant');
  ...
  await page.screenshot({ path: `${SS_DIR}/020_form_filled.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SS_DIR}/020_result.png`, fullPage: false });
  ...
  console.log('\n═══════════════════════════════════════════════════════');
  if (isSuccess) {
    console.log(' SUCCESS: Vacant property created in Dashboard!');
    console.log(' Address : ' + TEST_ADDRESS);
    console.log(' Status  : Vacant');
    ...
    console.log(' FAILED: Still on /new — check 020_result.png for errors.');
    process.exitCode = 1;
  }
  console.log('═══════════════════════════════════════════════════════');
