# HTM Clone Screenshot Test - HTM_Clone_Screenshot_003
...
- **Test ID**: HTM_Clone_Screenshot_003
...
## Screenshot
![HTM Clone Google OAuth - CAPTCHA Blocker](HTM_Clone_Screenshot_003.png)

## Blocker Analysis
Google detects headless Chromium sessions and presents CAPTCHA challenges to prevent automated login. To resolve, stored auth state (Playwright `storageState`) is required. Steps:

1. Open `https://admin-clone.helpthemove.co.uk` in your browser while logged in as `mac.murapa@helpthemove.co.uk`
2. Open DevTools Console and run:
```javascript
copy(JSON.stringify({
  cookies: document.cookie.split(';').map(c => {
    const [name, ...rest] = c.trim().split('=');
    return { name: name.trim(), value: rest.join('='), domain: location.hostname, path: '/' };
  }),
  origins: [{
...
```
3. Paste the copied JSON back — auth state will be saved as `auth.json` and reused in future runs
